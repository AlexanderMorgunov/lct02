...
export default class AuthEndPoint {
  ...
  static login = async (requestData: ILoginRequestData): Promise<ICurrentUser | null> => {
    const { data } = await $reqApi.post<IGetCurrentUserResponse, AxiosResponse<IGetCurrentUserResponse>, ILoginRequestData>('/auth/login', requestData);
    return data.status === "ok" ? data.data : null;
  }
}
